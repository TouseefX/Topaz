...
use ast::{ self };
use cfg::{ block::{ BlockEdge, BranchType }, function::Function };
...
impl<'a> Lifter<'a> {
    ...
    fn lift_function(&mut self) {
        ...
        let block_ranges = blocks
            ...
            .rev()
            .fold(
                (self.function_list[self.function.id].instructions.len(), Vec::new()),
                |(block_end, mut accumulator), &block_start| {
                    accumulator.push((block_start, block_end - 1));

                    (if block_start != 0 { block_start } else { block_end }, accumulator)
                }
            ).1;
        ...
        self.function.set_edges(
            entry_node,
            vec![(self.block_to_node(0), BlockEdge::new(BranchType::Unconditional))]
        );
    ...
    fn discover_blocks(&mut self) -> Result<()> {
        ...
        for (insn_index, insn) in self.function_list[self.function.id].instructions
            .iter()
            .enumerate() {
            match insn {
                Instruction::BC { op_code, c, .. } =>
                    match op_code {
                ...
                Instruction::AD { op_code, a: _, d, aux: _ } =>
                    match op_code {
                        | OpCode::LOP_JUMP
                        | OpCode::LOP_JUMPBACK
                        | OpCode::LOP_JUMPIF
                ...
                Instruction::E { op_code, e } => {
                    if *op_code == OpCode::LOP_JUMPX {
                        ...
                        self.blocks
                            .entry(insn_index + 1)
                            .or_insert_with(|| self.function.new_block());
                        self.blocks.entry(dest_index).or_insert_with(|| self.function.new_block());
                    }
                }
            }
        }
    ...
    fn lift_block(
        ...
        while let Some((index, instruction)) = iter.next() {
            match *instruction {
                Instruction::BC { op_code, a, b, c, aux } =>
                    match op_code {
                        ...
                        OpCode::LOP_MOVE => {
                            ...
                            statements.push(
                                ast::Assign::new(vec![a.into()], vec![b.into()]).into()
                            );
                        }
                        OpCode::LOP_GETUPVAL => {
                            let a = self.register(a as _);
                            ...
                            statements.push(
                                ast::Assign::new(vec![a.into()], vec![up.into()]).into()
                            );
                        }
                        OpCode::LOP_SETUPVAL => {
                            let a = self.register(a as _);
                            let up = self.upvalues[b as usize].clone();
                            statements.push(
                                ast::Assign::new(vec![up.into()], vec![a.into()]).into()
                            );
                        }
                        OpCode::LOP_LOADNIL => {
                            ...
                            statements.push(
                                ast::Assign
                                    ::new(vec![target.into()], vec![ast::Literal::Nil.into()])
                                    .into()
                            );
                        }
                        OpCode::LOP_LOADB => {
                        ...
                        OpCode::LOP_RETURN => {
                            let values = if b != 0 {
                                (a..a + (b - 1)).map(|r| self.register(r as _).into()).collect()
                            } else {
                        ...
                        OpCode::LOP_NAMECALL => {
                            ...
                            assert!(
                                matches!(iter.next().unwrap().1, Instruction::BC {
                                    op_code: OpCode::LOP_NOP,
                                    ..
                                })
                            );
                            match iter.next().unwrap().1 {
                                &Instruction::BC { op_code: OpCode::LOP_CALL, a, b, c, .. } => {
                        ...
                        OpCode::LOP_CALL => {
                            let arguments = if b != 0 {
                                (a + 1..a + b).map(|r| self.register(r as _).into()).collect()
                            } else {
                                let top = top.take().unwrap();
                                (a + 1..top.1)
                                    .map(|r| self.register(r as _).into())
                        ...
                        OpCode::LOP_SETLIST => {
                            let setlist = if c != 0 {
                                ast::SetList::new(
                                    ...
                                    aux as usize,
                                    (b..b + c - 1).map(|r| self.register(r as _).into()).collect(),
                                    None
                                )
                            } else {
                                let top = top.take().unwrap();
                        ...
                        OpCode::LOP_CONCAT => {
                            ...
                            statements.push(
                                ast::Assign
                                    ::new(vec![self.register(a as _).into()], vec![concat.into()])
                                    .into()
                            );
                        }
                        OpCode::LOP_AND =>
                ...
                Instruction::AD { op_code, a, d, aux } =>
                    match op_code {
                        ...
                        OpCode::LOP_GETIMPORT => {
                            ...
                            let import_len = (aux >> 30) + 1;

                            let base_idx = ((aux >> 20) & 0x3ff) as usize;
                            let mut import_expression: ast::RValue = ast::Global
                                ::new(self.constant(base_idx).into_string().unwrap())
                                .into();

                            if import_len >= 2 {
                                let field1_idx = ((aux >> 10) & 0x3ff) as usize;
                                import_expression = ast::Index
                                    ::new(import_expression, self.constant(field1_idx).into())
                                    .into();
                            }

                            if import_len >= 3 {
                                let field2_idx = (aux & 0x3ff) as usize;
                                import_expression = ast::Index
                                    ::new(import_expression, self.constant(field2_idx).into())
                                    .into();
                            }

                            let assign = ast::Assign::new(
                                vec![target.into()],
                                vec![import_expression]
                            );
                        ...
                        OpCode::LOP_JUMPIFNOTEQ => {
                            ...
                            statements.push(
                                ast::If
                                    ::new(
                                        ast::Binary
                                            ::new(a.into(), aux.into(), ast::BinaryOperation::Equal)
                                            .into(),
                                        ast::Block::default(),
                                        ast::Block::default()
                                    )
                        ...
                        | OpCode::LOP_FORGPREP_NEXT => {
                            ...
                            let counter = self.register((a + 2) as _);
                            statements.push(
                                ast::GenericForInit::new(generator, state, counter).into()
                            );
                            let loop_index = (((block_start + index + 1) as isize) +
                                (d as isize)) as usize;
                            assert!(
                                matches!(
            ...
            !Self::is_terminator(self.function_list[self.function.id].instructions[last_index])
        {
            if last_index + 1 == self.function_list[self.function.id].instructions.len() {
                statements.push(
                    ast::Comment::new("warning: block does not return".to_string()).into()
                );
            } else {
                edges.push((
    ...
    fn constant(&mut self, index: usize) -> ast::Literal {
        let converted_constant = match
            self.function_list[self.function.id].constants.get(index).unwrap()
        {
        ...
        self.constant_map.entry(index).or_insert(converted_constant).clone()
    }
