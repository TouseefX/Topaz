...
use cfg::{
    function::Function,
    ssa::{ self, structuring::{ structure_conditionals, structure_jumps } },
};
        ...
        Bytecode::Chunk(chunk) => {
            ...
            let mut upvalues = lifted
                ...
                .map(|(ast_function, function, upvalues_in)| {
                    ...
                    let mut args = std::panic::AssertUnwindSafe(
                        Some((ast_function.clone(), function, upvalues_in))
                    );
...
fn decompile_function(
    ...
    let (local_count, local_groups, upvalue_in_groups, upvalue_passed_groups) = cfg::ssa::construct(
        &mut function,
        &upvalues_in
    );
    let upvalue_to_group = upvalue_in_groups
        .into_iter()
        .chain(upvalue_passed_groups.into_iter().map(|m| (ast::RcLocal::default(), m)))
    ...
    ssa::Destructor
        ::new(&mut function, upvalue_to_group, upvalues_in.iter().cloned().collect(), local_count)
        .destruct();
