...
impl GraphStructurer {
    ...
    fn fix_misplaced_continues(block: &mut ast::Block) {
        ...
        while i < block.len() {
            ...
            let valid =
                ...
                matches!(
                    block.get(*statements_to_move.last().unwrap()),
    ...
    fn apply_formatting_rules(block: &mut ast::Block) {
        while
            block
                .first()
                .map(|s| matches!(s, ast::Statement::Comment(_)))
                .unwrap_or(false)
        {
            block.remove(0);
        }
        ...
        while block.len() > 0 {
            ...
            let should_remove = if let Some(last) = block.last() {
                matches!(last, ast::Statement::Comment(_)) &&
                    !matches!(last, ast::Statement::Return(_))
            } else {
                false
            };
    ...
    fn validate_gotos(block: &ast::Block) -> Result<(), String> {
        ...
        fn collect_labels(
            block: &ast::Block,
            defined: &mut FxHashSet<ast::Label>,
            used: &mut FxHashSet<ast::Label>
        ) {
            for statement in &block.0 {
                match statement {
    ...
    fn try_convert_to_repeat_until(&mut self, entry: NodeIndex, body: NodeIndex) -> bool {
        ...
        if let Some(if_stmt) = body_block.last_mut().and_then(|s| s.as_if_mut()) {
            let condition = if back_is_then {
                ast::Unary
                    ::new(if_stmt.condition.clone(), ast::UnaryOperation::Not)
                    .reduce_condition()
            } else {
            ...
            *self.function.block_mut(entry).unwrap() = new_block;
            ...
            self.function.set_edges(
                ...
                vec![(
                    exit_target,
                    cfg::block::BlockEdge::new(cfg::block::BranchType::Unconditional),
                )]
            );
    ...
    fn insert_goto_for_edge(&mut self, edge: EdgeIndex) {
        ...
        if
            edge_data.branch_type == BranchType::Unconditional &&
        ...
        self.function.block_mut(goto_block).unwrap().push(ast::Goto::new(label.clone()).into());
    ...
    fn inline_table_assignments(block: &mut ast::Block) {
                                ...
                                if let Some(assign) = s.as_assign() {
                                        ...
                                        assign.right.len() == 1
                                    {
                                        if let Some(_index) = assign.left[0].as_index() {
                                            if
                                                let ast::RValue::Local(value_local) =
                                                    &assign.right[0]
                                            {
        ...
        let mut parent_inlines: FxHashMap<
            usize,
            Vec<(ast::RValue, ast::Table)>
        > = FxHashMap::default();
                                        ...
                                        if let Some(index) = future_assign.left[0].as_index() {
                                            if let ast::RValue::Local(index_local) = &*index.left {
                                                if index_local == parent_local {
                                                    if
                                                        let ast::RValue::Local(value_local) =
                                                            &future_assign.right[0]
                                                    {
                                                        if
                                                            let Some(table_to_inline) =
                                                                to_inline.get(value_local)
                                                        {
                                                            parent_inlines
                                                                .entry(idx)
                                                                .or_default()
                ...
                ast::Statement::If(if_stmt) => {
                    ...
                    if if_stmt.else_block.lock().is_empty() {
                        ...
                        if can_flatten {
                            ...
                            let combined_condition = ast::Binary
                                ::new(
                                    if_stmt.condition.clone(),
                                    nested_condition,
            ...
            if should_chain {
                ...
                if chain_count > 0 {
                    for j in (1..=chain_count).rev() {
                        ...
                        *current_else = parking_lot::Mutex
                            ::new(vec![next_if_stmt.into()].into())
                            .into();
                    }
                }
            }
