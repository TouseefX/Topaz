...
impl<'a> Liveness<'a> {
    fn explore_all_paths(
        ...
        while let Some(node) = stack.pop() {
            ...
            if
                block_liveness.defs.contains(variable) ||
            ...
            for pred in function.predecessor_blocks(node) {
                liveness.block_liveness
                    .get_mut(&pred)
                    .unwrap()
                    .live_sets.live_out.insert(variable.clone());
                stack.push(pred);
            }
        }
    }
    ...
    pub fn calculate(function: &'a Function) -> FxHashMap<NodeIndex, LiveSets> {
        ...
        for (node, block) in function.blocks() {
            ...
            for instruction in block.iter() {
                block_liveness.uses.extend(instruction.values_read().into_iter());
                block_liveness.defs.extend(instruction.values_written().into_iter());
            }
            for (pred, edge) in function.edges_to_block(node) {
                liveness.block_liveness
                    .get_mut(&node)
                    .unwrap()
                    .params.extend(edge.arguments.iter().map(|(k, _)| k));
                let block_liveness = liveness.block_liveness.entry(pred).or_default();
                block_liveness.arg_out_uses.extend(
                    edge.arguments.iter().flat_map(|(_, v)| v.values_read())
                );
            }
        }
        for node in function.graph().node_indices() {
            ...
            block_liveness.live_sets.live_in.reserve(
                block_liveness.params.len() +
                    block_liveness.uses.len().saturating_sub(block_liveness.defs.len())
            );
