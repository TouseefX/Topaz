...
use crate::{ block::{ BlockEdge, BranchType }, function::Function };
    ...
    if let Some(if_stat) = block.last_mut().and_then(|s| s.as_if_mut()) {
        if let Some(unary) = if_stat.condition.as_unary() && unary.operation == UnaryOperation::Not {
            ...
            if binary.left.as_literal().is_some() && binary.right.as_literal().is_none() {
                if_stat.condition = ast::Binary
                    ::new(*binary.right.clone(), *binary.left.clone(), match binary.operation {
    ...
    if let Some(r#if) = block.last().and_then(|s| s.as_if()) {
        ...
        let test_pattern = |second_conditional, other, other_args: FxHashMap<_, _>| {
            ...
            if let Some(second_conditional_if) = second_block.last().and_then(|s| s.as_if()) {
                if
                    second_conditional_successors.len() == 2 &&
                        ...
                        .exactly_one()
                {
                    if second_block.len() == 2 {
                        if let ast::Statement::Assign(assign) = &second_block[0] {
                            ...
                            if
                                values_written.len() == 1 &&
                                second_conditional_if.condition == values_written[0].clone().into()
                            {
                                let valid = if
                                    other_args.len() == 1 &&
                                    let Ok((_, ast::RValue::Local(local))) = edge_to_other
                                        .weight()
                                        .arguments.iter()
                                        .exactly_one() &&
        ...
        if
            function.predecessor_blocks(then_edge.target()).count() == 1 &&
            ...
            let Some((second_condition, assign)) = test_pattern(
                then_edge.target(),
                else_edge.target(),
                else_args
            )
        {
            ...
            if second_terminator.0.target() == else_edge.target() {
                ...
                Some(ConditionalSequencePattern {
                    ...
                    inverted: false,
                    final_condition: ast::Binary
                        ::new(first_condition, second_condition, ast::BinaryOperation::And)
                        .into(),
                })
            }
        } else if
            function.predecessor_blocks(else_edge.target()).count() == 1 &&
            ...
            )
        {
            ...
            if first_terminator.0.target() == second_terminator.0.target() {
                Some(ConditionalSequencePattern {
                    ...
                    inverted: false,
                    final_condition: ast::Binary
                        ::new(first_condition, second_condition, ast::BinaryOperation::Or)
                        .into(),
                })
            } else {
                Some(ConditionalSequencePattern {
...
pub fn structure_conditionals(function: &mut Function) -> bool {
    ...
    while let Some(node) = dfs.next(function.graph()) {
        ...
        if
            let Some(pattern) = match_conditional_sequence(function, node) &&
            ...
            &Some(pattern.second_node) != function.entry()
        {
            ...
            if pattern.inverted {
                ...
                removed_if.condition = ast::Unary
                    ::new(removed_if.condition.clone(), UnaryOperation::Not)
                    .reduce_condition();
            }
...
fn is_truthy(rvalue: ast::RValue) -> Option<bool> {
    match rvalue.reduce_condition() {
        ...
        ast::RValue::Unary(ast::Unary { operation: ast::UnaryOperation::Length, .. }) => Some(true),
        | ast::RValue::Literal(
...
fn make_bool_conditional(
    ...
    if
        let ast::RValue::Literal(ast::Literal::Boolean(then_value)) = then_value &&
        ...
        then_value != else_value
    {
        ...
        let then_truthy = match is_truthy(then_value.clone()) {
            ...
            None if !then_value.has_side_effects() => {
                let value = match &r#if.condition {
                    ast::RValue::Binary(
                        ast::Binary { right: box value, operation: ast::BinaryOperation::And, .. },
                    ) => value,
                    value => value,
                };
        ...
        let cond = if !then_truthy && !else_truthy {
            ...
            let cond = std::mem
                ::replace(&mut r#if.condition, ast::Literal::Nil.into())
                .reduce_condition();
            if
                let ast::RValue::Unary(
                    ast::Unary { box value, operation: ast::UnaryOperation::Not },
                ) = cond
            {
                std::mem::swap(&mut then_value, &mut else_value);
                value
...
fn structure_bool_conditional(function: &mut Function, node: NodeIndex) -> bool {
    let match_triangle = |assigner, next, next_args: FxHashMap<ast::RcLocal, ast::RValue>| {
        if
            let Some(edge_to_next) = function.unconditional_edge(assigner) &&
            edge_to_next.target() == next &&
            edge_to_next
                .weight()
                .arguments.iter()
                .all(|(p, _)| next_args.contains_key(p)) &&
            ...
            next_args.len() == 1 &&
            let Ok((param, ast::RValue::Local(arg))) = edge_to_next
                .weight()
                .arguments.iter()
                .exactly_one() &&
            arg == assigned_local
    ...
    if let Some(ast::Statement::If(_)) = function.block(node).unwrap().last() {
        ...
        if then_edge.target() == else_edge.target() {
            if
                let Ok((res_local, then_value, else_value)) = then_edge
                    .weight()
                    .arguments.iter()
                    .filter_map(|(p, a)| {
                        else_edge
                    ...
                    .exactly_one()
            {
                ...
                if let Some(res) = make_bool_conditional(function, node, then_value, else_value) {
                    function.graph_mut().edge_weight_mut(then_edge).unwrap().arguments[0].1 =
                        res_local.clone().into();
                    function.graph_mut().edge_weight_mut(else_edge).unwrap().arguments[0].1 =
                        res_local.clone().into();
                    let block = function.block_mut(node).unwrap();
                    let r#if = block.last_mut().unwrap().as_if_mut().unwrap();
            ...
            let else_args = else_edge
                ...
                .cloned()
                .collect::<FxHashMap<_, _>>() &&
            let Some((res_local, then_value, else_value)) = match_triangle(
                then_edge.target(),
                else_edge.target(),
                else_args
            )
        {
            ...
            let res_local = res_local.clone();
            if let Some(res) = make_bool_conditional(function, node, then_value, else_value) {
                function.graph_mut().edge_weight_mut(then_edge).unwrap().arguments[0].1 = res_local
                    .clone()
                    .into();
                function.graph_mut().edge_weight_mut(else_edge).unwrap().arguments[0].1 = res_local
                    .clone()
                    .into();
            ...
            let then_args = then_edge
                ...
                .cloned()
                .collect::<FxHashMap<_, _>>() &&
            let Some((res_local, else_value, then_value)) = match_triangle(
                else_edge.target(),
                then_edge.target(),
                then_args
            )
        {
            ...
            // TODO: allow multiple unused (excl. first) locals in left
            then_assign.left.len() == 1 &&
            then_assign.right.len() == 1 &&
            let Some(else_assign) = single_assign(function.block(else_edge.target()).unwrap()) &&
            // TODO: allow multiple unused (excl. first) locals in left
            else_assign.left.len() == 1 &&
            else_assign.right.len() == 1 &&
            let Ok((then_param, ast::RValue::Local(then_arg))) = then_edge
                .weight()
                .arguments.iter()
                .exactly_one() &&
            let Ok((else_param, ast::RValue::Local(else_arg))) = else_edge
                .weight()
                .arguments.iter()
                .exactly_one() &&
            ...
            else_assign.left[0].as_local() == Some(else_arg)
        {
            ...
            function.predecessor_blocks(then_target).exactly_one().is_ok() &&
            function.predecessor_blocks(else_target).exactly_one().is_ok() &&
            function.successor_blocks(then_target).next().is_none() &&
            function.successor_blocks(else_target).next().is_none() &&
            let Ok(ast::Statement::Return(ast::Return { values: then_values })) = function
                .block(then_target)
                .unwrap()
                .iter()
                .exactly_one() &&
            let Ok(then_value) = then_values.iter().exactly_one() &&
            let Ok(ast::Statement::Return(ast::Return { values: else_values })) = function
                .block(else_target)
                .unwrap()
                .iter()
                .exactly_one() &&
...
fn match_method_call(call: &ast::Call) -> Option<(&ast::RValue, &str)> {
    ...
    if
        ...
        let Some(
            ast::Index { box left, right: box ast::RValue::Literal(ast::Literal::String(index)) },
        ) = call.value.as_index() &&
        left == &call.arguments[0]
    {
        if let Ok(index) = std::str::from_utf8(index) { Some((left, index)) } else { None }
    } else {
        None
    }
}
...
pub fn structure_method_calls(function: &mut Function) -> bool {
    ...
    for block in function.blocks_mut() {
        for stat in &mut block.0 {
            ...
            stat.traverse_rvalues(
                &mut (|rvalue| {
                    if let ast::RValue::Call(call) = rvalue {
                        if let Some((value, method)) = match_method_call(call) {
                            *rvalue = ast::MethodCall
                                ::new(
                                    value.clone(),
                                    method.to_string(),
                                    call.arguments.drain(1..).collect()
...
fn skip_over_node(
    ...
    let mut did_structure = false;
    let skip_to_after_args = function.graph().edge_weight(skip_to_after).unwrap().arguments.clone();
    for edge in function
        ...
        .map(|e| e.id())
        .collect::<Vec<_>>() {
        let mut new_arguments = function.graph().edge_weight(edge).unwrap().arguments.clone();
...
fn try_remove_unnecessary_condition(function: &mut Function, node: NodeIndex) -> bool {
        ...
        then_edge.weight().arguments == else_edge.weight().arguments
    {
        ...
        let cond = function.block_mut(node).unwrap().pop().unwrap().into_if().unwrap().condition;
...
fn is_for_next(function: &Function, node: NodeIndex) -> bool {
    function
        ...
        .unwrap()
        .first()
        .map(|s| { matches!(s, ast::Statement::GenericForNext(_) | ast::Statement::NumForNext(_)) })
        .unwrap_or(false)
}
...
pub fn structure_jumps(function: &mut Function, dominators: &Dominators<NodeIndex>) -> bool {
    ...
    for node in function.graph().node_indices().collect_vec() {
            ...
            !is_for_next(function, jump_target)
        {
            ...
            if
                function.predecessor_blocks(jump_target).count() == 1 &&
                dominators
