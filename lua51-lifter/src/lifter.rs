...
impl<'a, 'b> Lifter<'a, 'b> {
    fn compute_register_names(&self) -> Vec<Option<String>> {
        ...
        for (i, local) in locals.iter().enumerate() {
            let reg = locals[..i]
                .iter()
                .filter(|prev| prev.range.end > local.range.start)
                .count();
            ...
            let name_str = std::str::from_utf8(local.name).ok().unwrap_or("");
            if !name_str.is_empty() && !name_str.starts_with('(') && names[reg].is_none() {
                names[reg] = Some(name_str.to_string());
            }
        }
    ...
    fn for_loop_internal_registers(&self) -> rustc_hash::FxHashSet<u8> {
        ...
        for insn in &self.bytecode.code {
            if let Instruction::InitNumericForLoop { control, .. }
            | Instruction::IterateNumericForLoop { control, .. } = insn
            {
                internals.insert(control[0].0);
                internals.insert(control[1].0);
                internals.insert(control[2].0);
            }
            if let Instruction::IterateGenericForLoop {
                generator,
                state,
                internal_control,
                ..
            ...
            {
                internals.insert(generator.0);
                internals.insert(state.0);
                internals.insert(internal_control.0);
            }
        }
        internals
    }
    ...
    fn allocate_locals(&mut self) {
        ...
        for i in 0..self.bytecode.number_of_upvalues as usize {
            let name = self
                .bytecode
                .upvalues
                .get(i)
                ...
                .map(|n| n.to_string());
            self.upvalues
                .push(RcLocal::new(ast::Local::new(name)));
        }

        let register_names = self.compute_register_names();

        self.locals
            .reserve(self.bytecode.maximum_stack_size as usize);
        for i in 0..self.bytecode.maximum_stack_size {
            let name = register_names.get(i as usize).cloned().flatten();
