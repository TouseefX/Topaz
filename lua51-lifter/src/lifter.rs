...
impl<'a, 'b> Lifter<'a, 'b> {
    fn allocate_locals(&mut self) {
        self.upvalues.reserve(self.bytecode.number_of_upvalues as usize);
        for _ in 0..self.bytecode.number_of_upvalues {
            self.upvalues.push(RcLocal::default());
        }

        self.locals.reserve(self.bytecode.maximum_stack_size as usize);
        ...
        for (insn_index, insn) in self.bytecode.code.iter().enumerate() {
            match *insn {
                ...
                Instruction::LoadBoolean { skip_next: true, .. } => {
                    ...
                    self.nodes.entry(insn_index + 2).or_insert_with(|| self.function.new_block());
                }
                | Instruction::Equal { .. }
                ...
                Instruction::Jump(skip) => {
                    let dest_index = (insn_index + 1)
                        ...
                        .unwrap();
                    self.nodes.entry(dest_index).or_insert_with(|| self.function.new_block());
                    self.nodes.entry(insn_index + 1).or_insert_with(|| self.function.new_block());
                }
                ...
                | Instruction::InitNumericForLoop { skip, .. } => {
                    self.nodes
                        .entry(
                            (insn_index + 1).checked_add_signed(skip.try_into().unwrap()).unwrap()
                        )
    ...
    fn lift_instruction(&mut self, start: usize, end: usize, statements: &mut Vec<Statement>) {
        ...
        while let Some(instruction) = iter.next() {
            match instruction {
                ...
                &Instruction::LoadBoolean { destination, value, .. } => {
                    statements.push(
                        ast::Assign
                            ::new(
                                vec![self.locals[&destination].clone().into()],
                ...
                &Instruction::GetIndex { destination, object, key } => {
                    statements.push(
                        ast::Assign
                            ::new(
                                vec![self.locals[&destination].clone().into()],
                ...
                Instruction::Not { destination, operand } => {
                    statements.push(
                        ast::Assign
                            ::new(
                                vec![self.locals[destination].clone().into()],
                ...
                Instruction::Length { destination, operand } => {
                    statements.push(
                        ast::Assign
                            ::new(
                                vec![self.locals[destination].clone().into()],
                ...
                Instruction::Minus { destination, operand } => {
                    statements.push(
                        ast::Assign
                            ::new(
                                vec![self.locals[destination].clone().into()],
                ...
                Instruction::Jump(..) => {}
                | &Instruction::Add { destination, lhs, rhs }
                | &Instruction::Sub { destination, lhs, rhs }
                | &Instruction::Mul { destination, lhs, rhs }
                | &Instruction::Div { destination, lhs, rhs }
                | &Instruction::Mod { destination, lhs, rhs }
                | &Instruction::Pow { destination, lhs, rhs } => {
                    statements.push(
                        ast::Assign
                            ::new(
                                vec![self.locals[&destination].clone().into()],
                ...
                &Instruction::LessThanOrEqual { lhs, rhs, invert } => {
                    let lhs = self.register_or_constant(lhs);
                    let rhs = self.register_or_constant(rhs);
                    let value = ast::Binary
                        ::new(lhs, rhs, ast::BinaryOperation::LessThanOrEqual)
                        .into();
                    let condition = if invert {
                        ast::Unary::new(value, ast::UnaryOperation::Not).into()
                    } else {
                ...
                Instruction::TestSet { destination, value, invert } => {
                    ...
                    statements.push(
                        ast::Assign
                            ::new(
                                ...
                                vec![
                                    ast::Index
                                        ::new(object.into(), self.register_or_constant(method))
                                        .into()
                                ]
                            )
                            .into()
                ...
                | &Instruction::TailCall { function, arguments }
                | &Instruction::Call { function, arguments, .. } => {
                    ...
                    if
                        let &Instruction::Call { return_values, .. } = instruction &&
                        return_values != 0
                    {
                        if return_values == 1 {
                ...
                Instruction::Closure { destination, function } => {
                    ...
                    for _ in 0..closure.number_of_upvalues {
                        let local = match iter.next().as_ref().unwrap() {
                            Instruction::Move { destination: _, source } =>
                                self.locals[source].clone(),
                            Instruction::GetUpvalue { destination: _, upvalue } =>
                ...
                &Instruction::IterateNumericForLoop { ref control, skip } => {
                    ...
                    statements.push(
                        ast::NumForNext
                            ::new(internal_counter.clone(), limit.into(), step.into())
                            .into()
                    );

                    let body_node = self.get_node(
                        &(end + 1).checked_add_signed(skip.try_into().unwrap()).unwrap()
                    );
                    assert!(
                        self.insert_between
                            .insert(self.nodes[&start], (
                                body_node,
                                ast::Assign
                                    ::new(
    ...
    fn lift_blocks(&mut self) {
        ...
        for (start, end) in ranges {
            ...
            let mut statements = std::mem::take(
                self.function.block_mut(self.nodes[&start]).unwrap()
            );
    ...
    pub fn lift(
        ...
        for (node, (successor, stat)) in context.insert_between {
            if context.function.predecessor_blocks(successor).count() == 1 {
                context.function.block_mut(successor).unwrap().insert(0, stat);
            } else {
                ...
                for edge in context.function
                    ...
                    context.function.graph_mut().add_edge(node, between_node, edge);
                }
            }
        }
